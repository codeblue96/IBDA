...
export class AiMlComponent {
  ...
  ngOnInit(): void {
    this.fetchSliderImages('services-ai-machine-learning');
    this.fetchSliderImages('industries-ai-machine-learning');
  }
  ...
  fetchSliderImages(category: string): void {
    ...
    this.globalApiService.fetchSliderImageUrl(category).subscribe({
      next: (sliderImgs: GlobalSlider[]) => {
        ...
        if (category === 'industries-ai-machine-learning') {
        } else if (category === 'services-ai-machine-learning') {
